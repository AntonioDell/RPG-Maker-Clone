...
onready var _SpaceBar_Icon = find_node("SpaceBar_Label")

onready var _Option_Button_Scene = load("res://DialogSystem/scenes/Option.tscn")
...
func _ready():
	...
	#var story = load("res://DialogSystem/stories/Example_Story_Temp_Baked.tres")
	var story = load("res://KillTheOldDude/stories/Story_Baked.tres")
	...
	play_dialog("OldGuy/RantAboutSon")
...
func _play_node():
	...
	# TODO: support setting variables to certain values, when node is played
	# f.e. with <set>{"KEY": "VALUE"}</set>
...
func _populate_choices(json_text : String):
	var choices : Dictionary = parse_json(json_text)
...
func _on_Body_Label_custom_effects_finished():
